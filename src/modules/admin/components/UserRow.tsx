...
export function UserRow({ user, isSelf }: { user: User; isSelf: boolean }) {
  ...
  return (
    <div className="rounded-lg border border-white/15 bg-card-background p-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-medium">
            {user.name} {isSelf && <span className="text-white/40">(you)</span>}
          </p>
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="rounded-md border border-white/15 bg-white/5 px-2 py-1 text-sm disabled:opacity-50"
          >
